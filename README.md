# 💼 Portfolio de Justine BLIN

Portfolio professionnel présentant mes projets, mes compétences et mon parcours en développement informatique et gestion de projets.

🔗 **[Voir le site en ligne](https://justineb24.github.io/Portfolio/)**

---

## 📋 Table des matières

- [À propos](#-à-propos)
- [Fonctionnalités](#-fonctionnalités)
- [Technologies utilisées](#-technologies-utilisées)
- [Contact](#-contact)

---

## 👋 À propos

Actuellement étudiante en **Bachelor Chef de Projet Développement et IA** à La Manu (Lacroix-Saint-Ouen), je me spécialise dans le développement d'applications et la gestion de projets informatiques. Passionnée par la technologie et la création d'outils numériques, j'ai choisi cette filière pour approfondir mes compétences techniques et découvrir les bonnes pratiques du développement logiciel.

**Parcours :**

- 🎓 Bachelor Développement & IA (2025-2026)
- 📚 BTS SIO - SLAM (2023-2025)
- 🏆 Baccalauréat Général (Maths, NSI) (2020-2023)

---

## ✨ Fonctionnalités

- **Design Responsive** : Adapté à tous les appareils (mobile, tablette, desktop)
- **Mode Sombre/Clair** : Thème personnalisable
- **Menu Burger** : Navigation mobile fluide
- **Animations Fluides** : Transitions et animations CSS modernes
- **Timeline Interactive** : Parcours scolaire et professionnel présenté visuellement
- **Certificats** : Galerie des certifications
- **Liens Dynamiques** : Navigation vers les réseaux sociaux et projets

---

## 🛠 Technologies utilisées

### Frontend / Backend

- **HTML** - Structure sémantique
- **CSS** - Animations, Flexbox, Grid
- **JavaScript** - Interactivité et dynamique

### Outils

- **Visual Studio Code** - Éditeur de code
- **Git** - Versionning
- **FontAwesome** - Icônes
- **Google Fonts** - Typographie

---

## 📞 Contact

- **LinkedIn** : [Profil LinkedIn](https://www.linkedin.com/in/justine-blin-a88600292)
- **GitHub** : [Profil GitHub](https://github.com/JustineB24)
- **Email** : [blin.justine.sio@gmail.com](mailto:blin.justine.sio@gmail.com)

---

## ✍️ Auteur

**Justine BLIN**  
Étudiante en Bachelor Chef de Projet Développement et IA  
La Manu, Lacroix-Saint-Ouen
